...
Note this uploads the report to Google's temporary public storage (`lighthouserc.json`'s `upload.target`) — the same thing CI does on every PR, and the site is already public, but it's worth knowing this "local" check has that one external side effect.

Expected: exits 0, with `categories:accessibility` scoring at least the `lighthouserc.json`-configured `0.9` threshold (`ci.assert.assertions["categories:accessibility"]`). If this fails, don't just re-run it — read which element(s) Lighthouse actually flagged. Lighthouse's default run only audits the page's *resting*, unfiltered state, so the failure candidates worth checking first are ones visible without ever clicking a filter chip: `.project-meta`/`.project-tech` (`--text-muted` on `--surface`, the thinnest new contrast margin this feature adds — measured ~5.68:1 against the 4.5:1 floor), `.project-link` (`--accent-strong` on `--surface`, ~5.4:1), heading order in the two new sections (`h2` → `h3`, no skipped levels), and accessible names on the five new links. `.project-card.is-zero-match h3` is **not** a resting-state candidate — that class only exists while a filter is actively applied, which this default run never triggers — so don't spend time chasing it here; if you specifically want to check the filtered state too, trigger a filter with DevTools open and re-run Lighthouse's accessibility audit while it's active.
